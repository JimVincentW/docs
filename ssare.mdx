---
title: 'SSARE 🌐📰🔍🧠🗺️🪡📊🤖🗄️🔧'
description: 'An open-source service for political intelligence and news analysis'
---
...
Access the dashboard at http://localhost:8089/ or http://main_core_app:8089/ when inside the same Docker network.
Architecture
SSARE uses a microservices architecture for scalability and maintainability:
<AccordionGroup>
  ...
  <Accordion icon="server" title="API Service">
    Provides endpoints for querying and retrieving processed data.
  </Accordion>
</AccordionGroup>
Data Flow

Scraper scripts collect articles
Articles are processed for vector representation and entity extraction
Entities are geocoded if they represent locations
Data is stored in PostgreSQL and vector databases
API endpoints allow for querying and retrieval of processed data

Adding Sources
To add a new source to SSARE:

Create a scraping script in the scraper_service/scrapers folder
...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes Orchestration" icon="diagram-project">
